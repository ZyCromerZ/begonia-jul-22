...
static void smblib_check_init_boot_work(struct work_struct *work)
{
	struct charger_manager *pinfo = container_of(work, struct charger_manager, check_init_boot.work);
				...
				enum charger_type chr_type)
{
	...
	if (!first_boot_flag)
		schedule_delayed_work(&info->check_init_boot, msecs_to_jiffies(18000));
...
static bool mtk_is_charger_on(struct charger_manager *info)
{
	...
	chr_err("%s: charger_online=%d, chr_type=%d\n",
			__func__, charger_online, chr_type);
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&info->check_init_boot, smblib_check_init_boot_work);
	info->init_done = true;
	_wake_up_charger(info);
